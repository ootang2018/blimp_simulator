...
class FC:
    ...
    def __init__(self, output_dim, input_dim=None,
        ...
        self.decays = None
        # super hacky
        self._name = str(self.input_dim) + "_" + str(self.output_dim) + \
            "_" + str(datetime.datetime.now()).replace(':', "-").replace(" ", "-")

    def __repr__(self):
        return "FC(output_dim={!r}, input_dim={!r}, activation={!r}, normalization={!r}, weight_decay={!r}, ensemble_size={!r})"\
            .format(
                self.output_dim, self.input_dim, self.activation,
                self.normalization, self.weight_decay, self.ensemble_size
            )
    ...
    def compute_output_tensor(self, input_tensor, bm_train=False, name=None):
        ...
        if self.normalization is not None and self.normalization is not "none":
            ...
            if len(shape) == 3:
                with tf.variable_scope("", reuse=tf.AUTO_REUSE):
                    new_output = normalizer(
                        FC._activations[self.activation](
                            tf.reshape(raw_output, [-1, shape[2]])
                        ),
                        name_scope=name
                    )
                return tf.reshape(new_output, [shape[0], -1, shape[2]])
            else:
                assert len(shape) == 2
                return normalizer(FC._activations[self.activation](raw_output),
                                  name_scope=name)
        else:
